...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleAccessDeniedException(AccessDeniedException ex, WebRequest request) {
        ...
        if (acceptHeaderValue.contains(MediaType.TEXT_XML_VALUE)
            ...
            apiError.setMessage(
                    "Access denied. If you think you should have access to this resource, please contact support.");
            apiError.setDebugMessage(ex.getMessage());
            return buildResponseEntity(apiError);
        }
    }

    @ExceptionHandler(CrossTenantOperationException.class)
    protected ResponseEntity<Object> handleCrossTenantOperationException(CrossTenantOperationException ex, WebRequest request) {
        logger.error("Cross Tenant Operation Exception, ", ex);
        String acceptHeaderValue = request.getHeader("Accept");
        if (acceptHeaderValue == null || acceptHeaderValue.trim().isEmpty()) {
            acceptHeaderValue = "";
        }
    ...
    protected ResponseEntity<Object> handleTransactionSystemException(TransactionSystemException ex, WebRequest request) {
        logger.error("TransactionSystem Exception, ", ex);
        String acceptHeaderValue = request.getHeader("Accept");
        if (acceptHeaderValue == null || acceptHeaderValue.trim().isEmpty()) {
            acceptHeaderValue = "";
        }
        ...
        if (acceptHeaderValue.contains(MediaType.TEXT_XML_VALUE)
            ...
            if (ex.getRootCause() instanceof CrossTenantOperationException) {
                builder.path("/error403.html");
            } else {
